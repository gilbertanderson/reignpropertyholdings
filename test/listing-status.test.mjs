...
// the label actually appear) needs a browser and isn't covered here — see
// the PR description for how it was verified visually.
...
// --- Markup: status label present, Rent always live regardless of it ------
...
for (const [name, html] of [['index.html', index], ['properties.html', properties]]) {
  ...
  // The Rent button always links through TurboTenant, live regardless of
  // leased/available state — /apply/<slug> falls back to the contact form
  // gracefully, so there is nothing left to hide it behind.
  t(`${name}: 508 card's Rent link is not slug-tagged for hiding`, !/data-listing-apply/.test(html));
  t(`${name}: 508 card has an always-present Rent link`, /href="\/apply\/508-avenue-e" class="btn btn-outline btn-sm">Rent</.test(html));
  // 508 has no furnished-stay listing (functions/_shared/stays.js) — its
  // Book control is a permanently disabled button, not a live link.
  t(`${name}: 508 card's Book is a disabled button, not a link`, /<button type="button" class="btn btn-outline btn-sm" disabled>Book<\/button>/.test(html));
  // The two must NOT be nested inside a shared [data-listing-status] wrapper
  // that the old (buggy) version of this feature used — main.js queries the
  // status label independently, so a leftover wrapper attribute would be
  // dead markup.
...
t('detail page: no stray data-listing-apply wrapper left over', !/data-listing-apply/.test(detail));
t('detail page: Rent always links to TurboTenant, unconditionally', /href="\/apply\/508-avenue-e" class="btn btn-primary">Rent</.test(detail));
t('detail page: Book is a disabled button', /<button type="button" class="btn btn-outline" disabled>Book<\/button>/.test(detail));

// --- main.js wiring ----------------------------------------------------------
const mainJs = readFileSync(join(publicDir, 'js', 'main.js'), 'utf8');
t('main.js queries the status label (no shared apply-hiding lookup)', /data-listing-status-label/.test(mainJs) && !/data-listing-apply/.test(mainJs));
